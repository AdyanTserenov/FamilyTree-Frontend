...
> **Стек:** React 19 · TypeScript 5.9 · Vite 7 · TanStack Query v5 · Zustand 5 · ReactFlow 12 · Tailwind CSS v4  
...
- Анализировать биографию с помощью YandexGPT AI с голосовым вводом
...
Browser
  ...
  │
  └── axios (authApi)  ──→  auth-service  :8081  /auth-api/*
        └── JWT Bearer token в каждом запросе
```
...
│   │       └── VoiceInputButton.tsx  # Кнопка голосового ввода (AI-вкладка)
│   ├── hooks/
...
│   │   ├── PersonPage.tsx        # Карточка персоны (5 вкладок)
...
│   │   └── treeLayout.ts         # Кастомный алгоритм раскладки графа
│   ├── test/
...
### Кастомный алгоритм раскладки

Файл: [`src/utils/treeLayout.ts`](src/utils/treeLayout.ts)

Раскладка реализована **без сторонних библиотек** (не dagre). Алгоритм:

1. **BFS по поколениям** — обход графа в ширину для определения уровня (поколения) каждого узла
2. **Позиционирование по поколениям** — узлы одного поколения выравниваются по одной горизонтальной (TB) или вертикальной (LR) оси
3. **Paired slots** — партнёры размещаются в спаренных слотах через `personPartner` map, чтобы CoupleNode оказался между ними
4. **Дедупликация рёбер** — `seenChildEdges` Set предотвращает дублирование рёбер PARENT_CHILD, когда оба партнёра являются родителями одного ребёнка

**Константы раскладки:**

| Константа | Значение | Описание |
|-----------|----------|----------|
| `NODE_WIDTH` | 140 | Ширина узла персоны (px) |
| `NODE_HEIGHT` | 100 | Высота узла персоны (px) |
| `PRIMARY_GAP` | 120 | Горизонтальный отступ между узлами |
| `GENERATION_GAP` | 220 | Вертикальный отступ между поколениями |
...
**Реализация фильтрации через opacity:**

Не совпадающие узлы **не удаляются** из графа — они скрываются через CSS:
```typescript
style: { opacity: 0, pointerEvents: 'none' }
```

- `matchingPersonIds: Set<string> | null` вычисляется через `useMemo`
- `CoupleNode` скрывается только когда **оба** партнёра отфильтрованы
- Все рёбра сохраняются (`filteredEdges = initialEdges`) — граф не перестраивается
...
| Вкладка | Ключ | Содержимое |
|---------|------|-----------|
| **Основное** | `info` | ФИО, даты, места, профессия (occupation), биография, аватар |
| **Медиа** | `media` | Загрузка/просмотр/скачивание файлов; фильтр по типу (Все / Фото / Видео / Аудио / Документы); lazy loading изображений; счётчик (до 50 шт.) |
| **Комментарии** | `comments` | Добавление, редактирование, удаление комментариев |
| **История** | `history` | Лог изменений полей персоны (кто, когда, что изменил) |
| **AI-анализ** | `ai` | Голосовой ввод биографии → отправка в YandexGPT → список извлечённых событий с кнопкой «Вставить в биографию» |

**Вкладка AI-анализ подробнее:**
- [`VoiceInputButton`](src/components/ui/VoiceInputButton.tsx) размещён непосредственно в этой вкладке
- Результат анализа — список событий (дата + описание)
- Кнопка **«Вставить в биографию»** для каждого события добавляет его текст в поле биографии персоны
...
Кнопка голосового ввода на основе Web Speech API. Используется во вкладке **AI-анализ** страницы персоны. Поддерживает русский язык (`lang: 'ru-RU'`).
...
2. Деплоит на VM `158.160.55.234` через SSH
